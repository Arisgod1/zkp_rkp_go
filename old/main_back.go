...
package main // 有这行，go build 会跳过此文件
...
func main() {
	...
	{
		auth.POST("/register", rateLimitMiddleware(rdb, ctx, "zkp:ratelimit:register", 30, time.Minute), func(c *gin.Context) {
		...
		auth.POST("/challenge", rateLimitMiddleware(rdb, ctx, "zkp:ratelimit:challenge", 60, time.Minute), func(c *gin.Context) {
		...
		auth.POST("/verify", rateLimitMiddleware(rdb, ctx, "zkp:ratelimit:verify", 60, time.Minute), func(c *gin.Context) {
